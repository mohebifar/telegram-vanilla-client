...
import { MethodInfo, Usability } from "./methods";
...
const fromLine = (
  ...
  return new TLObject(
    ...
    match[2],
    argsMatch.map(
      ([brace, name, argType]) => new TLArg(name, argType, brace !== undefined)
    ),
    match[3],
    isFunction,
    usability,
 ...
 */
export const findLayer = filePath => {
